# from https://github.com/simplify23/MRN/blob/23b24806219d66722c916db49c823f96368beb49/modules/svtr.py
import numpy as np
import torch
...
class Block(nn.Module):
    ...
    def flops(self):
        flops = (
            self.attn.flops()
            + self.N * self.C * 2
            + 2 * self.N * self.C * self.C * self.mlp_ratio
        )
        return flops
...
class PatchEmbed(nn.Module):
    ...
    def flops(self):
        Ho, Wo = self.img_size
        flops = (
            Ho // 2 * Wo // 2 * 3 * self.embed_dim // 2 * (3 * 3)
...
class SVTR(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if block_unit == "CSWinBlock":
            ...
            ex_arg = [{"epsilon": epsilon}, {"epsilon": epsilon}, {"epsilon": epsilon}]
        ...
        if patch_merging is not None:
            self.sub_sample1 = SubSample(
                ...
                stride=[2, 1],
                types=patch_merging,
            )  # ConvBNLayer(embed_dim[0], embed_dim[1], kernel_size=3, stride=[2, 1], sub_norm=sub_norm)
        ...
        if patch_merging is not None:
            self.sub_sample2 = SubSample(
                ...
                stride=[2, 1],
                types=patch_merging,
            )  # ConvBNLayer(embed_dim[1], embed_dim[2], kernel_size=3, stride=[2, 1], sub_norm=sub_norm)
    ...
    def forward_features(self, x, tpsnet=None):
        ...
        if self.patch_merging is not None:
            x = self.sub_sample1(
                x.transpose(1, 2).reshape(B, self.embed_dim[0], self.HW[0], self.HW[1])
            )
        ...
        if self.patch_merging is not None:
            x = self.sub_sample2(
                x.transpose(1, 2).reshape(
                    B, self.embed_dim[1], self.HW[0] // 2, self.HW[1]
                )
            )
        ...
        if self.patch_merging is not None:
            ...
            x = self.sub_sample3(
                x.transpose(1, 2).reshape(
                    B, self.embed_dim[2], self.HW[0] // 4, self.HW[1]
                )
            )
        x = x.permute(0, 2, 1).reshape(
            [-1, self.out_channels, self.HW[0] // 8, self.HW[1]]
        )
        return x
